...
def predict_sales(df):
    ...
    future_df = pd.DataFrame({
        "Дата": pd.date_range(df["Дата"].max() + pd.Timedelta(days=1), periods=30),
        "Прогноз продаж": future_sales
    })
...
def export_to_pdf(recommendations, future_df):
    ...
    # Проверяем, есть ли шрифт в корневой папке
    if os.path.exists(font_path):
        pdf.add_font("DejaVu", "", font_path, uni=True)
        pdf.set_font("DejaVu", size=12)
    else:
        pdf.set_font("Arial", size=12)  # Если шрифт отсутствует, используем Arial
